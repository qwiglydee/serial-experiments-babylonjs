...
function buildAnglesAndRadius(position: Vector3, target: Vector3) {
    // the same math as in ArcRotateCamera.rebuildAnglesAndRadius
    let vector = position.subtract(target);
...
export function interpolateTarget(camera: ArcRotateCamera, newtarget: Vector3) {
    let { alpha, beta, radius } = buildAnglesAndRadius(camera.position, newtarget);
